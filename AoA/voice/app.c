...
static void sw_timer_event_handler(sl_bt_msg_t* evt);
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      ...
      if ( CONF_get()->remote_address_set ) {
        sc = sl_bt_system_set_soft_timer(SW_TIMER_20_MS, EVT_CONNECT, 1);
        app_assert(sc == SL_STATUS_OK,
                   "[E: 0x%04x] Failed to set soft timer\n",
                   (int)sc);
      } else {
        DEBUG_INFO("Scanning for VoBLE devices. ");
        sc = sl_bt_system_set_soft_timer(SW_TIMER_50_MS, EVT_SCANNING, 0);
        app_assert(sc == SL_STATUS_OK,
                   "[E: 0x%04x] Failed to set soft timer\n",
                   (int)sc);
    ...
    case sl_bt_evt_connection_closed_id:
      ...
      sc = sl_bt_advertiser_start(0, advertiser_general_discoverable, advertiser_connectable_scannable);
      app_assert(sc == SL_STATUS_OK,
    ...
    case sl_bt_evt_gatt_procedure_completed_id:
      ...
      if (conn_state == set_encoding && char_handle[char_transfer] != CHARACTERISTIC_HANDLE_INVALID) {
        ...
        uint8_t txfer_flags = gatt_disable;
        if (CONF_get()->transfer_status) {
          txfer_flags = gatt_notification;
        }
    ...
    // -------------------------------
    // This event is triggered when a software timer expired
...
static void connect(bd_addr remote_address)
{
  ...
  sc = sl_bt_connection_open(remote_address, /* le_gap_address_type_public = */ 0, gap_1m_phy, &ble_connection);
  if (sc != SL_STATUS_OK) {
...
static void emit_timeout_event(void)
{
  ...
  if ( !(scan_timout_counter--) ) {
    sl_status_t sc = sl_bt_system_set_soft_timer(SW_TIMER_20_MS, EVT_SCAN_TIMEOUT, 1);
    app_assert(sc == SL_STATUS_OK,
               "[E: 0x%04x] Failed to set soft timer\n",
               (int)sc);
  }
...
static void disableTimer(void)
{
  sl_status_t sc = sl_bt_system_set_soft_timer(0, EVT_SCANNING, 1);
  app_assert(sc == SL_STATUS_OK,
             "[E: 0x%04x] Failed to set soft timer\n",
             (int)sc);
}
...
static void emit_connect_or_timeout_event(void)
{
  ...
  if (SCAN_Is_Device_Found()) {
    disableTimer();
    sl_status_t sc = sl_bt_system_set_soft_timer(SW_TIMER_20_MS, EVT_CONNECT, 1);
    app_assert(sc == SL_STATUS_OK,
               "[E: 0x%04x] Failed to set soft timer\n",
               (int)sc);
  } else {
